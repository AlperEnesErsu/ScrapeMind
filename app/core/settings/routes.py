...
def security_setup_begin():
    ...
    if current_user.is_totp_enabled:
        return _render_tab("security", flash_msg=_("2FA is already enabled."),
                           flash_kind="warning", **_security_ctx())
...
def security_enable():
    ...
    if not pending:
        return _render_tab("security",
                           flash_msg=_("Setup expired — start again."),
                           flash_kind="danger", **_security_ctx())
...
def security_disable():
    ...
    if not form.validate_on_submit():
        return _render_tab("security", flash_msg=_("Code required."),
                           flash_kind="danger", **_security_ctx())

    code = (form.code.data or "").strip()
    if not (verify_totp(current_user.totp_secret, code)
            or consume_recovery_code(current_user, code)):
        return _render_tab("security", flash_msg=_("Code is invalid."),
                           flash_kind="danger", **_security_ctx())

    disable_totp(current_user)
    log_action("user.totp_disabled", entity_type="user", entity_id=current_user.id)
    return _render_tab("security", flash_msg=_("2FA disabled."),
                       flash_kind="success", **_security_ctx())
