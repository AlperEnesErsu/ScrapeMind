...
| ~~7~~ | ~~`ask_paper` "RAG" değil~~ | **Kapandı (PR #50).** Soru vektörleştirilip kütüphaneden en yakın makaleler bağlama giriyor; PR #108'den beri yalnızca geçerli embedding modelinin vektörleri karşılaştırılıyor |
...
| 10 | Canlı patent araması yalnızca başlık+özet tarar | EPO `ti,ab any`, PatentsView `_text_any` — hâlâ böyle. **Faz 8 bunu yerel korpus için kapattı:** son ~3 haftanın ABD patentlerinde istem ve tarifname düzeyinde arama `/patents/search`'te ([PHASE8.md](PHASE8.md)); prior-art sayfası oraya bağlanıyor. Canlı arama dünya çapında ve saklamıyor, yerel korpus ABD ve derin |
