...
| 🟡 Orta | 13 (**O1, O2, O3, O4, O5, O7, O9, O10, O11, O12, O13 kapandı**) | Planlanmalı, çıkışı engellemez |
...
| ~~O7~~ ✅ | ~~Python sürüm farkı~~ | **Kapandı — kök sebep `setup.bat`'taymış.** Prod ve CI zaten 3.11'di; venv `python -m venv venv` ile, yani makinedeki varsayılan Python'la (3.14) kuruluyordu. `setup.bat` artık venv'i **açıkça 3.11** ile kuruyor (`uv venv --seed --python 3.11`, yoksa `py -3.11`); 3.11 bulamazsa kurmayı reddediyor. `setup/development/worker.bat` isteğe bağlı `SCRAPEMIND_VENV` değişkenini okuyor (repo dışında bir venv için), venv'i **aktive etmeden önce** var olup olmadığını kontrol ediyor — eskiden sonra kontrol ediyordu ve olmayan venv'de `python`/`pip` sessizce sistem Python'una gidiyordu — ve venv 3.11 değilse uyarıyor. Mevcut bir 3.14 `venv` kendiliğinden değişmez: silip `setup.bat`'ı yeniden çalıştırmak ya da `SCRAPEMIND_VENV` vermek gerekir |
...
| ~~O11~~ ✅ | ~~Yerel mypy ile CI mypy aynı sonucu vermiyor~~ | **Kapandı, O7 ile.** Ölçüldü: temiz `origin/main` üzerinde 3.11 venv **75** (CI ile aynı), 3.14 venv **78**. Baseline PR #112'de 75'e indiği için 3.14 venv'de ratchet artık sebepsiz yere **düşüyordu** — fark "güvenilmez" olmaktan "yanlış alarm"a dönmüştü |
