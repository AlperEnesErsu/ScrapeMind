...
| ~~O7~~ ✅ | ~~Python sürüm farkı~~ | **Kapandı — iki parça, PR #114 + #115.** Prod ve CI zaten 3.11'di; venv `setup.bat`'ta makinedeki varsayılan Python'la (3.14) kuruluyordu. **#114:** `setup.bat` venv'i açıkça 3.11 ile kuruyor (`uv venv --seed --python 3.11`, yoksa `py -3.11`), 3.11 yoksa reddediyor; `setup/development/worker.bat` isteğe bağlı `SCRAPEMIND_VENV` okuyor ve venv yoksa sistem Python'una düşmüyor. **#115 — mypy farkının asıl nedeni:** Python sürümü değil, **venv'in `requirements.txt`'ten kayması**. CI imajında (`python:3.11-slim` + `requirements.txt`) mypy **75**; aynı 3.11 konteynerde yalnızca `beautifulsoup4`'ü eski venv'deki 4.15.0'a yükseltince **78** (pinli 4.12.3; üç hata da bs4'ün tip ipuçlarından). Yeni kurulan 3.11 venv pinli sürümlerle geldiği için 75 veriyor — ama zamanla kayarsa fark geri gelir. Eski venv'de 69 paket pinden farklıydı, aralarında **trafilatura 1.12 → 2.2** (büyük sürüm). Bu yüzden `mypy_ratchet.py` artık kaymış pin'leri adıyla uyarıyor |
...
| ~~O11~~ ✅ | ~~Yerel mypy ile CI mypy aynı sonucu vermiyor~~ | **Kapandı, O7 ile.** Baseline PR #112'de 75'e indiğinden beri eski venv'de ratchet sebepsiz **düşüyordu**. `python scripts/ci_local.py` (#115) CI'ın `lint-and-test` işini adım adım, aynı sırada, **Python 3.11** konteynerinde koşuyor (imaj `docker/ci.Dockerfile`, yalnızca `requirements.txt` değişince yeniden kurulur); konteyner CI'ın gördüğü ağacı görüyor (takip edilen + ignore edilmemiş dosyalar, commit'lenmemiş değişiklikler dahil; `.env`, `.mo`, venv hariç, kopyalanıyor — mount değil) ve her koşu kendi geçici pgvector veritabanını açıp kapatıyor, yani paylaşılan `scrapemind_test`'te eşzamanlı pytest kilitlenmesi de bitti. Doğrulama: 8/8 adım (1530 test), mypy 75 = CI; lint hatalı commit'lenmemiş dosya ruff'ta yakalandı, iş durdu (exit 1), geride konteyner/ağ kalmadı |
