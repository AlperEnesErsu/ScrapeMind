...
| ~~O7~~ ✅ | ~~Python sürüm farkı~~ | **Kapandı — ve teşhis iki kez yanlıştı.** Önce "prod ile CI ayrışıyor" yazıldı (değildi, ikisi de 3.11), sonra "yerel venv 3.14 olduğu için mypy ayrışıyor". O da değil: CI imajında (`python:3.11-slim` + `requirements.txt`) mypy **75**; aynı konteynerde yalnızca `beautifulsoup4`'ü yerel venv'deki 4.15.0'a yükseltince **78**. Fark **venv'in `requirements.txt`'ten kaymasıydı** (pinli 4.12.3; ayrıca gunicorn 23 → 26 ve **trafilatura 1.12 → 2.2** — büyük sürüm, yani geliştirme ortamı prod'dan farklı bir çıkarıcıyla çalışıyor; toplam 69 paket farklı). Python'u 3.11'e çekmek bu farkı kapatmazdı. Çözüm iki parça: `scripts/ci_local.py` CI işini 3.11 konteynerinde koşuyor, `mypy_ratchet.py` kaymış pin'leri adıyla uyarıyor. **Senin tarafında:** paylaşılan venv'de `pip install -r requirements.txt` (başka oturum da kullandığı için kendiliğinden yapılmadı) |
...
| ~~O11~~ ✅ | ~~Yerel mypy ile CI mypy aynı sonucu vermiyor~~ | **Kapandı.** `python scripts/ci_local.py` CI'ın `lint-and-test` işini adım adım, aynı sırada ve **Python 3.11** konteynerinde koşuyor (imaj `docker/ci.Dockerfile`, yalnızca `requirements.txt` değişince yeniden kurulur). Konteyner CI'ın gördüğü ağacı görüyor: git'in takip ettiği + ignore edilmemiş dosyalar, commit'lenmemiş değişiklikler dahil, `.env` / `.mo` / venv hariç; ağaç mount edilmiyor, kopyalanıyor. Her koşu kendi geçici pgvector veritabanını açıp kapatıyor — paylaşılan `scrapemind_test`'te iki oturumun pytest'inin kilitlenmesi de böylece bitiyor. Doğrulama: 8 adımın hepsi geçti (1530 test), mypy **75** = CI; commit'lenmemiş lint hatalı bir dosya ruff'ta yakalandı, iş orada durdu (exit 1), geride konteyner/ağ kalmadı |
